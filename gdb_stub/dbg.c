...
int gdbstub_main_loop(struct trap_state* ts, ssize_t* result, int* ern)
{
    ...
    for(;;)
    {
        ...
        case CMD_Q_OFFSETS:
        {
            ...
            base_addr += PAGE_SIZE;
#else
            // For PIE binaries, start_nonreloc == _start at runtime (the
            // dynamic linker applies the relocation), so the old formula
            // "base_addr -= (start_nonreloc - 0x400000)" always yields
            // 0x400000 regardless of the actual load address.
            //
            // Instead, scan backward page-by-page from _start looking for
            // the ELF magic "\x7FELF" (0x464C457F).  The ELF header is
            // always at the load base, and _start is within a few pages.
            {
                unsigned long long probe_addr = base_addr & ~((unsigned long long)(PAGE_SIZE-1));
                unsigned int magic;
                int found = 0;
                for(int i = 0; i < 16; i++, probe_addr -= PAGE_SIZE)
                {
                    if(read_mem((unsigned char*)&magic, probe_addr, 4))
                        break; // unmapped page, stop scanning
                    if(magic == 0x464C457F) // \x7FELF
                    {
                        base_addr = probe_addr;
                        found = 1;
                        break;
                    }
                }
                if(!found)
                {
                    // Last resort: original formula (broken for PIE, but
                    // correct for non-PIE linked at 0x400000).
                    static unsigned long long start_nonreloc = (unsigned long long)_start;
